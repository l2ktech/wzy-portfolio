...
document.querySelectorAll('.card').forEach(c=>{const v=c.querySelector('video'),pb=c.querySelector('.card-play');const sync=()=>{if(!v)return;const p=v.paused;if(pb)pb.classList.toggle('hidden',!p);v.controls=!p};v.addEventListener('play',sync);v.addEventListener('pause',sync);v.addEventListener('ended',sync);sync();const m=c.querySelector('.card-media');if(m)m.addEventListener('click',e=>{if(v.paused)v.play().catch(()=>{})});c.addEventListener('click',e=>{if(e.target.closest('a')||e.target.closest('.card-media'))return;location.hash='#project-'+c.id.replace('card-','')})})
</script>
</body>
</html>`
