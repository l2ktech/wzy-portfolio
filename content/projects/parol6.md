...
六轴驱动器只走 SocketCAN 的唯一执行入口，支持两种底层执行模式：**直通限速模式**——速度指令直通、仅做限幅；**梯形曲线加减速位置模式**——驱动器按梯形速度曲线自动完成加减速并到位，起停冲击小。

驱动器三环闭环反馈频率均为 **20 KHz 以上**：力矩环 20 KHz+、速度环 20 KHz+、位置环 20 KHz+。电机内部的高速闭环负责稳跟随，上层 50 Hz 的轨迹点只负责下发目标，两层周期职责分离。
