...
项目覆盖**硬件与接口集成、CAN 执行链、ROS 2 / MoveIt 2 规划执行、标定、RGB-D 抓取、安全门和真机调试验证**。机械臂本体、MoveIt 2、GR-ConvNet 和 VLM 为上游组件，系统重点是把它们集成为一套可运行、可排错、可验证的完整链路。
...
数字孪生可视化与测试在 Isaac Sim 中完成；同一模型的轨迹控制对照（纯 PID / PID+重力补偿 / 计算力矩）与失效注入实验在 MuJoCo 中进行。

手眼外参在跨姿态物理验证不足时不签发给运动链；多物体 VLM 抓取链已真机运行，鲁棒性仍在持续迭代。

项目把**底层执行、ROS 2 规划、视觉感知、语义模型和真机安全**连成一个系统，并支持沿执行链定位问题。
