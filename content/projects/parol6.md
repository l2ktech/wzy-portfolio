...
控制器对照实验里，自己生成六轴同步的**五次点到点轨迹** `s(u)=10u³−15u⁴+6u⁵`，同时算期望位置/速度/加速度，使起终点速度与加速度为零，再把 `qd / q̇d / q̈d` 分别交给 PID、重力补偿和计算力矩控制器跟踪。

这里把 MuJoCo 作为**动力学仿真和控制链验证工具**，不把重力补偿或计算力矩控制写成已经完成的真机控制能力。

### 6. 轨迹平滑与时间参数化

工业轨迹要“不抖”，轨迹连续性是底线。**梯形波 / S 型曲线**在起终点做加减速规划，**B 样条**把离散路径点作为控制点生成位置、速度、加速度连续的光滑曲线，避免直线连接在拐角处速度方向突变造成冲击。

同时把 **路径 → 插值 → 时间参数化** 三层职责分开：规划器给出 waypoint 路径，插值补中间状态，时间参数化决定每个状态何时到达（`time_from_start` 决定速度和加速度）。规划采样密度与硬件执行周期是两层职责：bridge 约 10ms 加密轨迹，底层链路约 20ms/50Hz，不能把规划频率、轨迹表示频率和底层控制周期混成一个概念。
